...
export default function ReportQuestionTeacher() {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const combined = questionsData.map(q => {
        ...
        const wrongCount = totalAttempted - correctCount; // student yang pernah menjawab tapi tidak pernah benar
        return {
          ...q,
  ...
  return (
    <div>
      ...
      <p>Statistik jawaban student per soal.</p>
          ...
          <tbody>
            {filteredQuestions.map((q, idx) => (
              <tr key={q.id}>
                ...
                <td>{q.chapter || "-"}</td>
                <td>{q.subchapter || "-"}</td>
                <td dangerouslySetInnerHTML={{ __html: q.question_text || "" }} style={styles.questionText} />

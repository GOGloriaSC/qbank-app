...
export default function StudentProfile() {
  ...
  useEffect(() => {
    document.title = "Profile | Aplikasiku";
    fetchUserAndProfile();
  }, []);
  ...
  const fetchUserAndProfile = async () => {
    ...
    if (user) {
      ...
      const { data: profileData, error } = await supabase
        ...
        .select("name")
        .eq("id", user.id)
        .maybeSingle(); // pakai maybeSingle() agar tidak error jika record belum ada
  ...
  const handleSave = async () => {
    ...
    if (existingProfile) {
      ...
      // INSERT: buat profil baru dengan role 'student'
      const { error: insertError } = await supabase
        .from("profiles")
        .insert({ id: user.id, name: profile.name, role: "student" });
      error = insertError;
    }
  ...
  if (loading) return <div style={styles.loading}>Memuat profil...</div>;

  return (
    <div>
      <h2>👤 Profil Saya</h2>
      <p>Informasi akun Anda sebagai student.</p>
      ...
      <div style={styles.note}>
        <small>💡 Untuk mengganti password, gunakan fitur "Lupa Password" di halaman login.</small>
      </div>
    </div>
  );
}

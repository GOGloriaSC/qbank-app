...
export default function AdminSetting() {
  ...
  useEffect(() => {
    document.title = "Pengaturan Admin | Aplikasiku";
    fetchAdminProfile();
  }, []);
  ...
  if (loading) return <div style={styles.loading}>Memuat pengaturan admin...</div>;

  return (
    <div>
      <h2>⚙️ Pengaturan Admin</h2>
      <p>Kelola keamanan akun Anda sebagai administrator.</p>

      <div style={styles.infoCard}>
        <h3>Informasi Akun</h3>
        <div style={styles.infoRow}><strong>Email:</strong> {profile.email}</div>
        <div style={styles.infoRow}><strong>Role:</strong> {profile.role === "admin" ? "Administrator" : "Pengguna"}</div>
